...
namespace :categories do
  ...
  task download: :environment do
    ...
    CATEGORIES.each do |k,v|
      CSV.open("materials/categories/itunes-podcast-category-lists-" + k.to_s + ".csv", "w") do |csv|
        ALPHABET.each do |letter|
          alpha_podcast_url = "#{v}&letter=#{letter}"

          # Determine how many pages there are.
          list_of_podcasts_doc = Nokogiri::HTML(open(alpha_podcast_url))
          number_of_pages = list_of_podcasts_doc.xpath("//*[@id='selectedgenre']/ul[3]/li/a").size
          number_of_pages |= 1
